...
def build_config(
    ...
    agents.extend(
        [
            ...
            for j in range(agent_count, agent_count + num_random_agents)
        ]
    )
    agent_count += num_random_agents
    agent_types.extend(["RandomAgent"])
    ...
    agents.extend(
        [
            PersistentAgent(
                id=j,
                name="PERSISTENT_AGENT_{}".format(j),
            ...
            for j in range(agent_count, agent_count + num_persistent_agents)
        ]
    )
    agent_count += num_persistent_agents
    agent_types.extend("PersistentAgent")
