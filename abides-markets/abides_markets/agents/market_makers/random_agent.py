...
#        self.avg_50_list: List[float] = []
    ...
    def receive_message(
        ...
        epsilon_ask = self.random_state.uniform(self.min_epsilon, self.max_epsilon)
        epsilon_bid = self.random_state.uniform(self.min_epsilon, self.max_epsilon)
        pct_to_hedge = self.random_state.uniform(0, 1)

        holdings = self.get_holdings(self.symbol)

        if holdings > 0:
            self.place_market_order(self.symbol,
                                    quantity=int(pct_to_hedge*holdings),
                                    side=Side.ASK,
                                    )
        else:
            self.place_market_order(self.symbol,
                                    quantity=int(pct_to_hedge*abs(holdings)),
                                    side=Side.BID
                                    )
        ...
        if (
            ...
            self.place_limit_order(self.symbol,
                                   quantity=self.size,
                                   side=Side.BID,
                                   limit_price=bid * (1+epsilon_bid)
                                   )
            self.place_limit_order(self.symbol,
                                   quantity=self.size,
                                   side=Side.ASK,
                                   limit_price=ask * (1+epsilon_ask)
                                   )
            ...
            if bids and asks:
                self.place_limit_order(self.symbol,
                                       quantity=self.size,
                                       side=Side.BID,
                                       limit_price=bids[0][0] * (1 + epsilon_bid)
                                       )
                self.place_limit_order(self.symbol,
                                       quantity=self.size,
                                       side=Side.ASK,
                                       limit_price=asks[0][0] * (1 + epsilon_ask)
                                       )
            self.state = "AWAITING_MARKET_DATA"
